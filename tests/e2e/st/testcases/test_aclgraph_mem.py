...
def test_aclgraph_mem_use(model: str, max_tokens: int) -> None:
    ...
    capture_called = multiprocessing.Value("i", 0)
    capture_mem_before = multiprocessing.Value("q", -1)
    capture_mem_after = multiprocessing.Value("q", -1)

    def capture_model_wrapper(original_method):
        def wrapped(self):
            mem_before = torch.npu.mem_get_info()[0]
