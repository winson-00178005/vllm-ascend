...
def test_aclgraph_logprobs_without_batch_invariance_should_fail(monkeypatch: pytest.MonkeyPatch):
    ...
    with VllmRunner(
        ...
        prompts: list[str] = []
        options = [
            (max(long_min, 1536), max(long_max, 3072)),
            (max(1024, long_min), max(2048, long_max)),
            (256, 512),
            (10, 20),
        ]
    ...
    if differences_found:
        success_msg = (
            f"âœ?SUCCESS: Batch invariance is doing something! "
        ...
        fail_msg = (
            f"âœ?UNEXPECTED: All {len(prompts)} prompts matched "
